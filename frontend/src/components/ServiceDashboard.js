...
const handleLogout = () => {
  ...
  const fetchPendingRequests = async () => {
    try {
      const res = await axios.get("https://vehicle-service-kfie.onrender.com/request/pending");
...
const fetchActiveRequest = async () => {
  try {
    const res = await axios.get(
      `https://vehicle-service-kfie.onrender.com/request/active/${user._id}`
    );
    ...
    async (pos) => {
      try {
        const res = await axios.put(
          `https://vehicle-service-kfie.onrender.com/request/accept/${requestId}`,
          {
        ...
        const fullReq = await axios.get(
          `https://vehicle-service-kfie.onrender.com/request/${requestId}`
        );
...
const handleConfirmCompletion = async (requestId) => {
  try {
    await axios.put(`https://vehicle-service-kfie.onrender.com/request/confirm-completion/${requestId}`);
  ...
  const startLocationUpdates = (requestId) => {
    setInterval(() => {
      navigator.geolocation.getCurrentPosition(async (pos) => {
        try {
          await axios.put(
            `https://vehicle-service-kfie.onrender.com/request/update-location/${requestId}`,
            {
              serviceManLat: pos.coords.latitude,
              serviceManLng: pos.coords.longitude,
            }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await axios.get(
          `https://vehicle-service-kfie.onrender.com/request/${activeRequest._id}`
        );
        setActiveRequest(res.data);
